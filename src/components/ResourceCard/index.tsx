      ...
      <div onClick={doSearch}>
        <Card.Meta
          ...
          description={
            <Tooltip
              title={
                <Paragraph ellipsis={{ rows: 3 }} type="secondary" style={{ color: '#fff', marginBottom: 0 }}>
                  <span dangerouslySetInnerHTML={{ __html: resource.desc ?? '' }} />
                </Paragraph>
              }
            >
              <div dangerouslySetInnerHTML={{ __html: resource.desc ?? '' }} />
            </Tooltip>
          }
        />
        <TagList resource={resource} showReview={showReview} style={{ minHeight: 40 }} />
      </div>
    </Card>
  );
};
